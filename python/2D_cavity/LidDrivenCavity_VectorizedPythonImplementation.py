# Python prototype of lid driven cavity code.
import math
import numpy as np
...
N = 251 # Number of Lattice Points along the characteristic direction
Num_ts = 20000 # number of simulation time steps

omega = 1.8 # relaxation parameter set this to ensure stable and consistent operation.
...
# Cavity is oriented from (0,Lx) in the x-direction and
# (0,Ly) in the y-direction.  The cavity surface at y = Ly
# is assigned a constant velocity in the x-direction
# consistent with the input Reynolds number.
# All other cavity boundaries are assumed stationary and rigid
# and the fluid obeys a no-slip boundary condition.
...
## form the data structure that will hold the lattice densities.  C is row-major and for the CUDA accelerated
# version we will want sequential threads accessing sequential memory addresses -- so we want
# all of the density distribution function values for a given velocity to reside on the same row
# (thus, in consecutive memory addresses)
...
# make assignment to moving vs solid nodes
