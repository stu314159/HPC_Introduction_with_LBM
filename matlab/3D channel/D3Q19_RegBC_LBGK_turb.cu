...
__global__ void D3Q19_RegBC_LBGK_ts(const double * fIn, double * fOut,
				    ...
				    const double omega, const double Cs,
  ...
  if(tid<nnodes){
   ...
   //speed 0, ex=ey=ez=0, w=1/3
	...
	f18=fe18+(f15-fe15);
      ...
      //compute off-equilibrium for boundary nodes
      ft1=f1-fe1; 
      ft2=f2-fe2;
      ft3=f3-fe3;
      ft4=f4-fe4;
    ...
    // need non-equilibribum component
    // f0 not needed since it doesn't show up in the strain tensor
    ft1=f1-fe1; 
    ft2=f2-fe2;
    ft3=f3-fe3;
    ft4=f4-fe4;
    ...
    // strain tensor symbolically determined by MATLAB (index decremented by 1)
    double S11 = ft1 + ft2 + ft7 + ft8 + ft9 + ft10 + ft11 + ft12 + ft13 + ft14;
    double S22 = ft3 + ft4 + ft7 + ft8 + ft9 + ft10 + ft15 + ft16 + ft17 + ft18;
    double S33 = ft5 + ft6 + ft11 + ft12 + ft13 + ft14 + ft15 + ft16 + ft17 + ft18;
    double S12 = ft7 - ft8 - ft9 + ft10;
    double S13 = ft11 - ft12 - ft13 + ft14;
    double S23 = ft15 - ft16 -ft17 + ft18;
    ...
    f0=f0-omega_t*(f0-fe0);
    f1=f1-omega_t*(f1-fe1);
    f2=f2-omega_t*(f2-fe2);
    f3=f3-omega_t*(f3-fe3);
    f4=f4-omega_t*(f4-fe4);
    f5=f5-omega_t*(f5-fe5);
    f6=f6-omega_t*(f6-fe6);
    f7=f7-omega_t*(f7-fe7);
    f8=f8-omega_t*(f8-fe8);
    f9=f9-omega_t*(f9-fe9);
    f10=f10-omega_t*(f10-fe10);
    f11=f11-omega_t*(f11-fe11);
    f12=f12-omega_t*(f12-fe12);
    f13=f13-omega_t*(f13-fe13);
    f14=f14-omega_t*(f14-fe14);
    f15=f15-omega_t*(f15-fe15);
    f16=f16-omega_t*(f16-fe16);
    f17=f17-omega_t*(f17-fe17);
    f18=f18-omega_t*(f18-fe18);
    }else{
      //bounce back
      f0=f0-omega_t*(f0-fe0);
      //1 -- 2
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  // get pointers to mx objects from argument list
  const mxGPUArray * const m_fIn = mxGPUCreateFromMxArray(prhs[0]);
  const mxGPUArray * const m_fOut = mxGPUCreateFromMxArray(prhs[1]);
  const mxGPUArray * const m_SNL = mxGPUCreateFromMxArray(prhs[2]);
  const mxGPUArray * const m_INL = mxGPUCreateFromMxArray(prhs[3]);
  const double u_lbm = mxGetScalar(prhs[4]);
  const mxGPUArray * const m_ONL = mxGPUCreateFromMxArray(prhs[5]);
  const double rho_out = mxGetScalar(prhs[6]);
  const double omega = mxGetScalar(prhs[7]);
  const double Cs = mxGetScalar(prhs[8]);
  const int Nx = mxGetScalar(prhs[9]);
  const int Ny = mxGetScalar(prhs[10]);
  const int Nz = mxGetScalar(prhs[11]);
  
  // get device pointers for the device arrays
  const double * d_fIn = (const double * )(mxGPUGetDataReadOnly(m_fIn));
  double * d_fOut = (double *)(mxGPUGetDataReadOnly(m_fOut));
  const int * d_SNL = (const int *)(mxGPUGetDataReadOnly(m_SNL));
  const int * d_INL = (const int *)(mxGPUGetDataReadOnly(m_INL));
  const int * d_ONL = (const int *)(mxGPUGetDataReadOnly(m_ONL));
  
  const int TPB = 64;
  int Blocks;
  
  int nnodes = Nx*Ny*Nz;
  Blocks = (nnodes+TPB-1)/TPB;
  
  D3Q19_RegBC_LBGK_ts<<<Blocks,TPB>>>(d_fIn,d_fOut,d_SNL,d_INL,u_lbm,
                                      d_ONL,rho_out,omega,Cs,Nx,Ny,Nz);
  
  
  // destroy mxGPUArray objects
  mxGPUDestroyGPUArray(m_fIn);
  mxGPUDestroyGPUArray(m_fOut);
  mxGPUDestroyGPUArray(m_SNL);
  mxGPUDestroyGPUArray(m_INL);
  mxGPUDestroyGPUArray(m_ONL);
